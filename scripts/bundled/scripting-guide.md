...
| `app.exportPdf(documents, path, options)` | Writes one PDF with a page per document (an array, or one document), each page sized from that document's pixels and resolution. Options: `lossless` (default true; false writes JPEG), `imageQuality` (`"lossless"`, `"high"`, `"medium"`, or `"low"`; wins over `lossless`), `editableLayers` (keep shapes and text as PDF objects, default false), `missingFontsAsImages`. Throws on bad arguments or a write failure. A folder of files becomes a PDF by opening each with `app.open` and passing the documents here. |
